...
// The 2D fallback orb — a soft, fluid gradient sphere on white. Three blurred
// color fields drift inside a clipped circle; the live audio level (real mic
// amplitude for the candidate, real playback amplitude for AudioContext
// engines, shaped envelope for speechSynthesis) drives scale, drift speed, and
// saturation. Cool pastels = interviewer, warm pastels = candidate, near-grey
// breathing = idle.
//
// This was the app's orb until the ElevenLabs WebGL orb replaced it, and it
// stays as the fallback for machines that cannot open a WebGL context — a lab
// PC or a locked-down laptop on demo day still gets a living orb instead of a
// hole in the page. VoiceOrb picks between the two; nothing else imports this.
...
export function VoiceOrbCanvas({
  ...
  /** Same contract as the WebGL orb: hidden or off-screen means stop drawing.
   * This path exists for the weakest machines, so it is the one that can least
   * afford to blur three gradient fields at 60fps into a canvas nobody sees. */
  paused?: boolean;
}) {
  ...
  hueRef.current = hue;
  // Read in the draw loop, so toggling it never restarts the animation.
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      if (pausedRef.current) {
        // Stay subscribed but paint nothing: the last frame remains on the
        // canvas, and resuming needs no re-setup.
